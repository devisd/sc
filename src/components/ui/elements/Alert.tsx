'use client';

import React from 'react';
import { clsx } from 'clsx';

type AlertSeverity = 'success' | 'info' | 'warning' | 'error';
type AlertVariant = 'standard' | 'outlined' | 'filled';

interface AlertProps {
    children: React.ReactNode;
    severity?: AlertSeverity;
    variant?: AlertVariant;
    action?: React.ReactNode;
    icon?: React.ReactNode;
    onClose?: () => void;
    closeText?: string;
    className?: string;
}

export const Alert: React.FC<AlertProps> = ({
    children,
    severity = 'info',
    variant = 'standard',
    action,
    icon,
    onClose,
    closeText,
    className = '',
    ...props
}) => {
    // Иконки по умолчанию в зависимости от severity
    const defaultIcons = {
        success: (
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
        ),
        info: (
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
            </svg>
        ),
        warning: (
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
        ),
        error: (
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
            </svg>
        ),
    };

    // Цвета в зависимости от severity и variant
    const variantClasses = {
        standard: {
            success: 'bg-green-50 text-green-800 border-l-4 border-green-500',
            info: 'bg-blue-50 text-blue-800 border-l-4 border-blue-500',
            warning: 'bg-yellow-50 text-yellow-800 border-l-4 border-yellow-500',
            error: 'bg-red-50 text-red-800 border-l-4 border-red-500',
        },
        outlined: {
            success: 'bg-white text-green-800 border border-green-500',
            info: 'bg-white text-blue-800 border border-blue-500',
            warning: 'bg-white text-yellow-800 border border-yellow-500',
            error: 'bg-white text-red-800 border border-red-500',
        },
        filled: {
            success: 'bg-green-600 text-white',
            info: 'bg-blue-600 text-white',
            warning: 'bg-yellow-600 text-white',
            error: 'bg-red-600 text-white',
        },
    };

    const alertIcon = icon || defaultIcons[severity];

    const closeButton = onClose && (
        <button
            onClick={onClose}
            className={clsx(
                "ml-auto -mx-1.5 p-1.5 rounded-full focus:outline-none focus:ring-2",
                variant === 'filled' ? 'hover:bg-white/10 focus:ring-white' : {
                    'success': 'hover:bg-green-100 focus:ring-green-400',
                    'info': 'hover:bg-blue-100 focus:ring-blue-400',
                    'warning': 'hover:bg-yellow-100 focus:ring-yellow-400',
                    'error': 'hover:bg-red-100 focus:ring-red-400',
                }[severity]
            )}
            aria-label={closeText || "Close alert"}
        >
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
            </svg>
        </button>
    );

    const alertClasses = clsx(
        'flex items-center p-4 mb-4 rounded',
        variantClasses[variant][severity],
        className
    );

    return (
        <div className={alertClasses} role="alert" {...props}>
            {alertIcon && <div className="mr-3 flex-shrink-0">{alertIcon}</div>}
            <div className="text-sm flex-grow">{children}</div>
            {action && <div className="ml-auto mr-2">{action}</div>}
            {onClose && closeButton}
        </div>
    );
};
